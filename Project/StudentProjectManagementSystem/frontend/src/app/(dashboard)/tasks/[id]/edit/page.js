...
export default function EditTaskPage() {
  ...
  const { getTaskById, updateTask, projects, users } = useData();
  ...
  useEffect(() => {
    if (existing) {
      setForm({ ...existing, dueDate: dayjs(existing.dueDate) });
    }
  }, [existing]);

  if (!form) return <Loader message="Loading task..." />;
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const newErrors = { title: validateRequired(form.title, 'Title') };
    setErrors(newErrors);
    ...
    <Box>
      <PageHeader
        ...
        breadcrumbs={[
          ...
          { label: form.title, href: `/tasks/${taskId}/edit` },
        ]}
      />
      ...
      <FormContainer>
        <Grid container spacing={2.5} component="form" onSubmit={handleSubmit}>
          ...
          <Grid size={12}>
            <TextField fullWidth label="Description" multiline rows={3} value={form.description} onChange={handleChange('description')} />
          </Grid>
